...
if( file_exists($conf['absolute_path_download']."files/".$folder."/.sender")){
	...
	$body = "<center><img src='".$conf['img_for_email']."'/></center><br/><p>".$vocables["$lang"]["sender_mail_1_part1"].$frommail.$vocables["$lang"]["sender_mail_1_part2"]."</p><a href=\"".$conf['url_redirection_download']."?id=".$tmp_var."&key=".$key."&iv=".$iv.">".$vocables["$lang"]["sender_mail_2"]."\">Download files</a>";
	mail($sender, utf8_decode($subject), utf8_decode($body), $entetemail);
}

file_put_contents($conf['absolute_path_download']."files/$folder/.sender",$frommail);
?>
