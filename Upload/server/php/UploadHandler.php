...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		$error_messages = array(
			1 => $vocables[$lang]["error_1"],
			2 => $vocables[$lang]["error_2"],
			3 => $vocables[$lang]["error_3"],
			4 => $vocables[$lang]["error_4"],
			6 => $vocables[$lang]["error_6"],
			7 => $vocables[$lang]["error_7"],
			8 => $vocables[$lang]["error_8"],
			'post_max_size' => $vocables[$lang]["post_max_size"],
			'max_file_size' => $vocables[$lang]["max_file_size"],
			'min_file_size' => $vocables[$lang]["min_file_size"],
			'accept_file_types' => $vocables[$lang]["accept_file_types"],
			'max_number_of_files' => $vocables[$lang]["max_number_of_files"],
			'max_width' => $vocables[$lang]["max_width"],
			'min_width' => $vocables[$lang]["min_width"],
			'max_height' => $vocables[$lang]["max_height"],
			'min_height' => $vocables[$lang]["min_height"],
			'abort' => $vocables[$lang]["abort"],
			'image_resize' => $vocables[$lang]["image_resize"],
			'invalid_arg' => $vocables[$lang]["invalid_arg"],
			'max_share' => $vocables[$lang]["max_share"],
			'max_upload' => $vocables[$lang]["max_upload"],
			'bad_folder' => $vocables[$lang]["bad_folder"],
		);
        if ($error_messages) {
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url']
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
            .'='.rawurlencode($file->name)."&folder=".$this->options['name_dir'];
    ...
    protected function get_error_message($error) {
        return array_key_exists($error, $this->error_messages) ?
            	$this->error_messages[$error] : "Invalid error";
    }
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
		...
		if(is_dir($dirname)) {
			...
			while($file = readdir($dir)) {
				$global_size = $global_size + filesize($dirname.'/'.$file);
			}
			$max_size = $conf['max_size'] - $global_size;
		} else {
    ...
    protected function upcount_name_callback($matches) {
        $index = isset($matches[1]) ? intval($matches[1][1]) + 1 : 1;
	...
	$index = null, $content_range = null) {
        ...
        $chiffre_data = $this->chiffre($file_path,$_REQUEST['iv'],$_REQUEST['key']);
		session_start();
		...
		syslog(LOG_INFO, ";ATT-UPLOAD;".date("d/m/Y-G:i").";".$_REQUEST['folder'].";".$_SERVER['REMOTE_ADDR'].";;".$file->name.";".formatSize($file_size).";".$type.";");
		closelog();
        return $file;
    }
    ...
    public function chiffre($dossier,$iv,$key){
		//d'abords on initialise l'objet de chiffrement
		/* Open the cipher */
		...
		* on Windows instead */
		...
		if(file_exists($dossier)){
			$file_handler=fopen($dossier, "r");
			if( !$file_handler){
				echo "erreur à l'ouverture du fichier à chiffrer";
			}
			else{
				$file_content = fread($file_handler,filesize($dossier));
				
				fclose($file_handler);
				$file_handler=fopen($dossier, "w");
